...
int map_sketch_sprite_allow_morph(ENTITY *ent, int type, int asset, int flag, int e_type, int e_id, int temp_skill)
{
    if (!ent)
    {
        return false;
    ...
    if (ent->OBJ_TYPE != type)
    {
        return true;
    }

    if (ent->OBJ_ASSET != asset)
    {
        return true;
    }

    if (ent->OBJ_ASSET != flag)
    {
        return true;
    }

    if (ent->OBJ_ASSET != e_type)
    {
        return true;
    }

    if (ent->OBJ_ASSET != e_id)
    {
        return true;
    }

    if (ent->OBJ_ASSET != temp_skill)
    {
        return true;
    }
...
void map_sketch_sprite_set_skill(ENTITY *ent, int type, int asset, int flag, int e_type, int e_id, int temp_skill)
{
    if (!ent)
    {
        return;
...
void map_sketch_sprite_update_from_cell(Cell *cell)
{
    ...
    {
        if (map_sketch_sprite_allow_morph(cell_ent, cell_type, cell_asset, cell_flag, cell_event_type, cell_event_id, cell_temp_skill) == true)
        {
            ent_morph(cell_ent, _chr(asset_get_filename(cell_type, cell_asset)));
            map_sketch_sprite_set_skill(cell_ent, cell_type, cell_asset, cell_flag, cell_event_type, cell_event_id, cell_temp_skill);
        }
