...
void vec_snap_to_grid(VECTOR *out)
{
    ...
    out->x += (MAP_CELL_SIZE / 2) * sign(out->x);
    out->x = (integer(out->x / MAP_CELL_SIZE) * MAP_CELL_SIZE);

    out->y += (MAP_CELL_SIZE / 2) * sign(out->y);
    out->y = (integer(out->y / MAP_CELL_SIZE) * MAP_CELL_SIZE);
...
void vec_worldpos_to_grid(VECTOR *worldpos, int *x, int *y)
{
    ...
    *x = floor(worldpos->x / MAP_CELL_SIZE);
    *x = clamp(*x, -1, MAP_WIDTH);

    *y = floor(-worldpos->y / MAP_CELL_SIZE);
