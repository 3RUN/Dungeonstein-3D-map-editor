...
BOOL imgui_h_file_browser(IMGUI_H_FILE_BROWSER *_fb)
{
	...
	int _i = 0;
	for (; _i < _txtDecomposed->skill_x; _i += 1)
	{ // _txtDecomposed->skill_x contains the current count of members
			...
			if (imgui_begin_combo("##DrivesCombo", _chrLogicalDrives[_fb->logicalDriveIndex], ImGuiComboFlags_NoArrowButton))
			{
				...
				for (; _ii < _nLogicalDrivesCount; _ii += 1)
				{
					...
					if (imgui_selectable(_chrLogicalDrives[_ii], &_isSelected, NULL))
					{
						...
						str_cat(strBrowserPath, "\\"); // add a counter bar to the logical drive, otherway it will point to the drive mounting p�int
		...
		{ // if it is not a logical drive
			sys_marker("300");
			if (_i != 0)
			{								   // if it is not the first member of the decomposed path
	...
	if (imgui_list_box("##PathContentList", &_fb->selectedIndex, &_fcBrower->content, _fcBrower->count, 6))
	{
		...
		sys_marker("500");
		if (imgui_h_path_is_folder_U(strBrowserPath->chars))
		{ // if the full path is a folder
			// Set the new current path
