...
STRING *get_cell_info(Cell *cell)
{
    ...
    if (cell_type >= 0 && cell_asset >= 0)
    {
        ...
        else if (cell_type == ASSET_TYPE_ENEMIES)
        {
            ...
            str_cat(info_str, editor_info_enemy_behaviour_str);
            if (cell_temp_skill == 0)
            {
                str_cat(info_str, editor_info_enemy_behaviour_wait_str);
            }
            else if (cell_temp_skill == 1)
            {
                str_cat(info_str, editor_info_enemy_behaviour_deaf_str);
            }
            else if (cell_temp_skill == 2)
            {
                str_cat(info_str, editor_info_enemy_behaviour_patrol_str);
            }
            str_cat(info_str, editor_semicolon_str);
